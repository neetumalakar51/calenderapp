// Program to accept any month and year and display the calendar.
// By:		Neetu & Alakeswari
// Date: 	01/05/2016
#include <stdio.h>
#include <conio.h>
...
void PrintFile(int M,int Y, int Z)
{
	...
	for(i=1;i<=MAX_NO;i++)
		fprintf(stream,"-");
	fprintf(stream,"\n\n\t\tCreated by: neeetu and alakeswari");
	fclose(stream);
}
...
Top:    /* goto statement */
	...
	do
	{
		...
		textcolor(11);
		textbackground(9);
		cprintf("Created by: Alakeswari Binduprava Patel and neetu kumari malakar ");
		textcolor(WHITE);
		textbackground(BLACK);
